...
public:
    ...
    */
    ...
    int8_t inJumpForDEP(uint16_t timeout = 0);
    int8_t inJumpForDEP(const uint8_t* command, const uint8_t len, const uint16_t timeout = 0);
    ...
    bool inDataWrite(const uint8_t *header, uint8_t headerLength, const uint8_t *body, uint8_t bodyLength);
    bool inDataRead(uint8_t *response, uint8_t *responseLength);
